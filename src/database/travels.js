...
export async function findTravelById(travelId, currentUserId) {
	...
	if (!travel) {
		throw generateError(404, "TravelNotFound", "Travel not found");
	}
...
export async function assertTravelExists(travelId) {
	...
	if (result.length == 0) {
		throw generateError(404, "TRAVEL_NOT_FOUND", "The travel does not exist");
	}
}
...
export async function assertUserIsTravelOwner(userId, travelId) {
	...
	if (!result) {
		throw generateError(
			403,
			"FORBIDDEN",
			"You are not the owner of this travel"
		);
	}
}
